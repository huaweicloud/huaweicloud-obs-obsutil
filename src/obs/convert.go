...
func convertGrantToXml(grant Grant) string {
	...
	if grant.Grantee.Type == GranteeUser {
		xml = append(xml, fmt.Sprintf("<ID>%s</ID>", XmlTranscoding(grant.Grantee.ID)))
		if grant.Grantee.DisplayName != "" {
			xml = append(xml, fmt.Sprintf("<DisplayName>%s</DisplayName>", XmlTranscoding(grant.Grantee.DisplayName)))
		}
	} else {
		xml = append(xml, fmt.Sprintf("<URI>%s</URI>", XmlTranscoding(string(grant.Grantee.URI))))
	}
	xml = append(xml, fmt.Sprintf("</Grantee><Permission>%s</Permission></Grant>", XmlTranscoding(string(grant.Permission))))
...
func ConvertLoggingStatusToXml(input BucketLoggingStatus, returnMd5 bool) (data string, md5 string) {
	...
	if input.TargetBucket != "" || input.TargetPrefix != "" {
		...
		xml = append(xml, fmt.Sprintf("<TargetBucket>%s</TargetBucket>", XmlTranscoding(input.TargetBucket)))
		xml = append(xml, fmt.Sprintf("<TargetPrefix>%s</TargetPrefix>", XmlTranscoding(input.TargetPrefix)))
	...
	if returnMd5 {
		md5 = Base64Md5(StringToBytes(data))
	}
	return
}
...
func ConvertAclToXml(input AccessControlPolicy, returnMd5 bool) (data string, md5 string) {
	...
	xml = append(xml, fmt.Sprintf("<AccessControlPolicy><Owner><ID>%s</ID>", XmlTranscoding(input.Owner.ID)))
	if input.Owner.DisplayName != "" {
		xml = append(xml, fmt.Sprintf("<DisplayName>%s</DisplayName>", XmlTranscoding(input.Owner.DisplayName)))
	}
...
func convertConditionToXml(condition Condition) string {
	...
	if condition.KeyPrefixEquals != "" {
		xml = append(xml, fmt.Sprintf("<KeyPrefixEquals>%s</KeyPrefixEquals>", XmlTranscoding(condition.KeyPrefixEquals)))
	}
	if condition.HttpErrorCodeReturnedEquals != "" {
		xml = append(xml, fmt.Sprintf("<HttpErrorCodeReturnedEquals>%s</HttpErrorCodeReturnedEquals>", XmlTranscoding(condition.HttpErrorCodeReturnedEquals)))
	}
	if len(xml) > 0 {
...
func ConvertWebsiteConfigurationToXml(input BucketWebsiteConfiguration, returnMd5 bool) (data string, md5 string) {
	...
	if input.RedirectAllRequestsTo.HostName != "" {
		xml = append(xml, fmt.Sprintf("<RedirectAllRequestsTo><HostName>%s</HostName>", XmlTranscoding(input.RedirectAllRequestsTo.HostName)))
		if input.RedirectAllRequestsTo.Protocol != "" {
			xml = append(xml, fmt.Sprintf("<Protocol>%s</Protocol>", XmlTranscoding(string(input.RedirectAllRequestsTo.Protocol))))
		}
		xml = append(xml, "</RedirectAllRequestsTo>")
	} else {
		xml = append(xml, fmt.Sprintf("<IndexDocument><Suffix>%s</Suffix></IndexDocument>", XmlTranscoding(input.IndexDocument.Suffix)))
		if input.ErrorDocument.Key != "" {
			xml = append(xml, fmt.Sprintf("<ErrorDocument><Key>%s</Key></ErrorDocument>", XmlTranscoding(input.ErrorDocument.Key)))
		}
		if routingRuleLength > 0 {
			...
			for _, routingRule := range input.RoutingRules {
				...
				if routingRule.Redirect.Protocol != "" {
					xml = append(xml, fmt.Sprintf("<Protocol>%s</Protocol>", XmlTranscoding(string(routingRule.Redirect.Protocol))))
				}
				if routingRule.Redirect.HostName != "" {
					xml = append(xml, fmt.Sprintf("<HostName>%s</HostName>", XmlTranscoding(routingRule.Redirect.HostName)))
				}
				if routingRule.Redirect.ReplaceKeyPrefixWith != "" {
					xml = append(xml, fmt.Sprintf("<ReplaceKeyPrefixWith>%s</ReplaceKeyPrefixWith>", XmlTranscoding(routingRule.Redirect.ReplaceKeyPrefixWith)))
				}

				if routingRule.Redirect.ReplaceKeyWith != "" {
					xml = append(xml, fmt.Sprintf("<ReplaceKeyWith>%s</ReplaceKeyWith>", XmlTranscoding(routingRule.Redirect.ReplaceKeyWith)))
				}
				if routingRule.Redirect.HttpRedirectCode != "" {
					xml = append(xml, fmt.Sprintf("<HttpRedirectCode>%s</HttpRedirectCode>", XmlTranscoding(routingRule.Redirect.HttpRedirectCode)))
				}
				xml = append(xml, "</Redirect>")
...
func convertTransitionsToXml(transitions []Transition) string {
	if length := len(transitions); length > 0 {
		...
		for _, transition := range transitions {
			...
			if temp != "" {
				xml = append(xml, fmt.Sprintf("<Transition>%s<StorageClass>%s</StorageClass></Transition>", temp, XmlTranscoding(string(transition.StorageClass))))
			}
		}
		return strings.Join(xml, "")
	}
		...
		for _, noncurrentVersionTransition := range noncurrentVersionTransitions {
			if noncurrentVersionTransition.NoncurrentDays > 0 {
				xml = append(xml, fmt.Sprintf("<NoncurrentVersionTransition><NoncurrentDays>%d</NoncurrentDays>"+
					...
					noncurrentVersionTransition.NoncurrentDays, XmlTranscoding(string(noncurrentVersionTransition.StorageClass))))
			}
		}
		return strings.Join(xml, "")
	}
...
func ConvertLifecyleConfigurationToXml(input BucketLifecyleConfiguration, returnMd5 bool) (data string, md5 string) {
	...
	for _, lifecyleRule := range input.LifecycleRules {
		xml = append(xml, "<Rule>")
		if lifecyleRule.ID != "" {
			xml = append(xml, fmt.Sprintf("<ID>%s</ID>", XmlTranscoding(lifecyleRule.ID)))
		}
		xml = append(xml, fmt.Sprintf("<Prefix>%s</Prefix>", XmlTranscoding(lifecyleRule.Prefix)))
		xml = append(xml, fmt.Sprintf("<Status>%s</Status>", XmlTranscoding(string(lifecyleRule.Status))))
...
func converntFilterRulesToXml(filterRules []FilterRule) string {
	if length := len(filterRules); length > 0 {
		...
		for _, filterRule := range filterRules {
			...
			if filterRule.Name != "" {
				xml = append(xml, fmt.Sprintf("<Name>%s</Name>", XmlTranscoding(filterRule.Name)))
			}
			if filterRule.Value != "" {
				xml = append(xml, fmt.Sprintf("<Value>%s</Value>", XmlTranscoding(filterRule.Value)))
			}
			xml = append(xml, "</FilterRule>")
		}
...
func converntEventsToXml(events []string) string {
	if length := len(events); length > 0 {
		...
		for _, event := range events {
			xml = append(xml, fmt.Sprintf("<Event>%s</Event>", XmlTranscoding(event)))
		}
		return strings.Join(xml, "")
	}
	return ""
...
func ConvertNotificationToXml(input BucketNotification, returnMd5 bool) (data string, md5 string) {
	...
	for _, topicConfiguration := range input.TopicConfigurations {
		...
		if topicConfiguration.ID != "" {
			xml = append(xml, fmt.Sprintf("<Id>%s</Id>", XmlTranscoding(topicConfiguration.ID)))
		}
		xml = append(xml, fmt.Sprintf("<Topic>%s</Topic>", XmlTranscoding(topicConfiguration.Topic)))
...
func ConvertCompleteMultipartUploadInputToXml(input CompleteMultipartUploadInput, returnMd5 bool) (data string, md5 string) {
	...
	for _, part := range input.Parts {
		...
		xml = append(xml, fmt.Sprintf("<ETag>%s</ETag>", XmlTranscoding(part.ETag)))
		xml = append(xml, "</Part>")
	}
	xml = append(xml, "</CompleteMultipartUpload>")
	data = strings.Join(xml, "")
...
func ParseGetObjectMetadataOutput(output *GetObjectMetadataOutput) {
	...
	if ret, ok := output.ResponseHeaders[HEADER_CONTENT_LENGTH]; ok {
		output.ContentLength = StringToInt64(ret[0], -1)
	} else {
...
func ParseGetBucketMetadataOutput(output *GetBucketMetadataOutput) {
	...
	if ret, ok := output.ResponseHeaders[HEADER_AZ_REDUNDANCY_OBS]; ok {
		output.AvailableZone = ParseStringToAvailableZoneType(ret[0])
	}
	if ret, ok := output.ResponseHeaders[HEADER_ACCESS_CONRTOL_ALLOW_ORIGIN]; ok {
		output.AllowOrigin = ret[0]
	}
...
func ParseDropFileOutput(output *DropFileOutput) {
	ParseDeleteObjectOutput(&output.DeleteObjectOutput)
}
...
func ConvertRequestToIoReaderV2(req interface{}) (io.Reader, string, error) {
	...
	if err == nil {
		if isDebugLogEnabled() {
			doLog(LEVEL_DEBUG, "Do http request with data: %s", BytesToString(data))
		}
...
func ConvertRequestToIoReader(req interface{}) (io.Reader, error) {
	...
	if err == nil {
		if isDebugLogEnabled() {
			doLog(LEVEL_DEBUG, "Do http request with data: %s", BytesToString(body))
		}
...
func ParseResponseToBaseModel(resp *http.Response, baseModel IBaseModel, xmlResult bool) (err error) {
	...
	if !ok {
		...
		if err == nil && len(body) > 0 {
			if isDebugLogEnabled() {
				doLog(LEVEL_DEBUG, "Get http response body: %s", BytesToString(body))
			}

			if xmlResult {
				err = ParseXml(body, baseModel)
				if err != nil && isErrorLogEnabled() {
				...
				for i := 0; i < s.NumField(); i++ {
					if s.Field(i).Tag == "body" {
						reflect.ValueOf(baseModel).Elem().FieldByName(s.Field(i).Name).SetString(BytesToString(body))
						break
					}
				}
			}
		...
		if err != nil {
			...
			if values, ok := responseHeaders[HEADER_REQUEST_ID]; ok {
				requestIds = append(requestIds, values[0])
				obsError.setRequestId(requestIds[0])
			}
			obsError.Message = err.Error()
			err = obsError
		}
	} else {
	...
	if values, ok := responseHeaders[HEADER_REQUEST_ID]; ok {
		requestIds = append(requestIds, values[0])
		baseModel.setRequestId(requestIds[0])
	}

	if isInfoLogEnabled() {
		if values, ok := responseHeaders[HEADER_REQUEST_ID2]; ok {
			requestIds = append(requestIds, values[0])
		}
		if len(requestIds) > 0 {
			doLog(LEVEL_INFO, "Request Ids: %s", strings.Join(requestIds, ", "))
		}
	}
...
func ParseResponseToObsError(resp *http.Response) error {
	obsError := ObsError{}
	parseError := ParseResponseToBaseModel(resp, &obsError, true)
	if parseError != nil && isWarnLogEnabled() {
		doLog(LEVEL_WARN, "Failed to parse response to BaseModel(ObsError) with reason: %v", parseError)
	}
