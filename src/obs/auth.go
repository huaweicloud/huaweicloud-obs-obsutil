...
func (obsClient ObsClient) doAuthTemporary(method, bucketName, objectKey, policy string, params map[string]string,
	...
	if sh.AK != "" && sh.SK != "" {
		if isV4 {
			...
			if policy == "" {
				...
				policy = Base64Encode(StringToBytes(policy))
				headers[HEADER_DATE_CAMEL] = []string{policy}
				canonicalizedUrl = ""
			}

			stringToSign := getV2StringToSign(method, canonicalizedUrl, headers)
			signature := UrlEncode(Base64Encode(HmacSha1(StringToBytes(sh.SK), StringToBytes(stringToSign))), false)
			...
			delete(headers, HEADER_DATE_CAMEL)
			if policy == "" {
				requestUrl += fmt.Sprintf("AWSAccessKeyId=%s&Expires=%d&Signature=%s", UrlEncode(sh.AK, false),
					expires, signature)
			} else {
				requestUrl += fmt.Sprintf("AWSAccessKeyId=%s&Policy=%s&Signature=%s", UrlEncode(sh.AK, false),
...
func (obsClient ObsClient) doAuth(method, bucketName, objectKey string, params map[string]string,
	headers map[string][]string, hostName string, needEncodeHeader, skipAuth bool) (requestUrl string, err error) {
	...
	delete(headers, HEADER_STS_TOKEN_AMZ)
	...
	if sh.AK == "" || sh.SK == "" {
		...
		if isV4 {
			...
			ret := v4Auth(sh.AK, sh.SK, obsClient.conf.region, method, canonicalizedUrl, parsedRequestUrl.RawQuery, headers)
			...
			ret := v2Auth(sh.AK, sh.SK, method, canonicalizedUrl, headers)
			authorization = fmt.Sprintf("%s %s:%s", V2_HASH_PREFIX, sh.AK, ret["Signature"])
		}
...
func prepareHostAndDate(headers map[string][]string, hostName string, isV4 bool) {
		...
		if len(date) == 1 {
			if isV4 {
				if t, err := time.Parse(LONG_DATE_FORMAT, date[0]); err == nil {
					requestDate = FormatUtcToRfc1123(t)
					flag = true
				}
			} else {
	...
	if requestDate == "" {
		requestDate = FormatUtcToRfc1123(time.Now().UTC())
	}

	headers[HEADER_DATE_CAMEL] = []string{requestDate}
}
...
func v2Auth(ak, sk, method, canonicalizedUrl string, headers map[string][]string) map[string]string {
	stringToSign := getV2StringToSign(method, canonicalizedUrl, headers)
	return map[string]string{"Signature": Base64Encode(HmacSha1(StringToBytes(sk), StringToBytes(stringToSign)))}
}
...
func getV4StringToSign(method, canonicalizedUrl, queryUrl, scope, longDate, payload string, signedHeaders []string, headers map[string][]string) string {
	...
	stringToSign = append(stringToSign, "\n")
	stringToSign = append(stringToSign, HexSha256(StringToBytes(_canonicalRequest)))
...
func getSignature(stringToSign, sk, region, shortDate string) string {
	key := HmacSha256(StringToBytes(V4_HASH_PRE+sk), StringToBytes(shortDate))
	key = HmacSha256(key, StringToBytes(region))
	key = HmacSha256(key, StringToBytes(V4_SERVICE_NAME))
	key = HmacSha256(key, StringToBytes(V4_SERVICE_SUFFIX))
	return Hex(HmacSha256(key, StringToBytes(stringToSign)))
}
